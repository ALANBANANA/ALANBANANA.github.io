...
int main()
{
    ...
    printf("11111111\n");
#endif
    printf("AAAAAAAAA\n");  // 这里面因为N == 0,所以编译的时候上面不走而走下面
}
```

其余和一般的条件相似，但是要在每个条件分支加一个 # endif

### 24.2 #ifdef #ifndef

> ifdef，表示如果定义了条件的宏，那么则将保留条件后的代码，否则不保留编译与执行
>
> ifndef，表示如果没有定义了条件的宏，那么将保留条件后的代码，否则定义的将不做编译与执行

## 二十五. 多文件代码

#include后的文件将放在你所主要打的代码的前面，像函数一样由上往下进行执行读写

1. #include<stdio.h>：仅会在编译器的包含目录内搜索文件
2. #include""：可以在文件目录内搜索文件，然后就是包含目录去搜索

另一类就是你在你需要编写代码的文件内进行声明，而你要的函数当作是一个外部文件，但是用声明函数来调用这个指令
...
类似python的调包，可以将一个函数写入一个头文件如**"function.h"**然后用#include传入到你要编写的主代码文件中

```c
#include"print.h"  // 头文件的调用
int main()
{
    print("cao\n");  // 这个则调用print.c文件
}
...
> 3. 源文件person.c需要头文件person.h中的声明或者定义，因此需要在源文件中加上#include “person.h”
> 4. 使用者，例如文件main.c包含头文件person.h后即可使用头文件中的声明或定义以及调用头文件中声明过的函数

具体以下网址可以方面查询单独的**.h文件当作单纯的函数声明，而.c文件当作单纯的函数文件**

<[菜鸟攻略——C语言多文件编程初探（一）-CSDN博客](https://blog.csdn.net/candcplusplus/article/details/7317472)>

## 二十六. 存储类别

一般的变量均属于自动变量，代码块开始命名之后存在，代码块结束之后自动销毁
...
{
	int n = 100;
    return &n;  // n的周期只限于在func内，代码结束则自动销毁
}
int main()
{
    int *pN = func();  // 于是这里就变成了野指针，在内存上属于一个空位置
    printf("%d\n", *pN);
    return 0;
}
```
...
int main()
{
    func();  // 假如之前加入printf函数，编译是会错误的，因为n具有块作用域
    func();
    func();
    return 0;
}
...
| ---------------- | -------- | ---------------------------------------- |
| 记录文件信息结构 | 文件位置 | 文件路径(相对路径或者绝对路径)，操作模式 |

两种操作模式：1. ’w‘写入模式，2.’r'读取模式

**fprintf函数**

> fprintf(pFile, "%d\n", n);  是将后面类似占位符的玩意写入到pFile里面的文件去

**fclose关闭文件操作**：fclose(文件流)

### 27.2 关于换行和回车操作

windows系统：文本直接具有\r和\n操作，所以故此叫做文本模式，在写C语言时的\n,实际上是\n\r的混合，但当读取文件时又变回了文本操作，写入文件时将会是\n\r，实际上有点自动操作的意思，读取文件时将会是\n

假如想要取消自动换行则需以下操作

```c
FIle *pFile = fopen("data.txt", "wb");
FIle *pFile = fopen("data.txt", "rb");  // 这里的b就是二进制操作，防止自动回车或者换行即单词binary
```
...
> fscanf(文件流，需要的读取格式，数据名地址)  -- >后面部分与scanf函数差不多

**fgetc函数**：单个字符读取文件，重复调用可以逐个读取文件内的字符

```c
int fgetc(File * stream)
```

> 读取文件中的数据，文件结尾或者读取失败，返回EOF(end of file)；读取成功，返回读取的到的字符
...
**fgets函数**：读取字符串的函数，重复调用可以逐行读取文件内的字符

```c
char * fgets(char*str, int num, File*stream)
```

char*str：读取的字符串保存在str内   int num：最大的字符串读取字符数    File *stream：文件六
...
>
> character：写入文件的字符  stream：文件结构指针
...
> 如果写入成功，返回一个非赋值；写入失败，返回EOF
>
> char*str待写入文件的字符串，stream：文件指针
...
> int：刷新成功；刷新失败返回EOF，返回0， FILE*stream：文件结构地址
...
> 1. FILE*stream：文件指针
> 2. long offset(数字)：文件指针偏移量
> 3. int origin(参数)：从什么位置开始偏移：(1)SEEK_SET:文件开头 (2)SEEK_CUR:当前文件位置 (3)SEEK_END:文件结尾
...
> 如果获取成功，返回当前文件的地址；失败返回-1  FILE*stream：指针的位置
...
> 1. w+：更新模式，可读可写。但是，会清空文件原有内容
> 2. r+：更新模式，可读可写，不会清空内容

**NOTICE**

> 1. 写转读——>必须使用fflush，fseek，rewind其中一个函数
> 2. 读转写——>必须使用fseek，rewind其中一个函数

仅需要读写转换，但无需变动指针可以使用**fseek(pFile, 0, SEEK_CUR)**

### 27.6 保存文件

**数值的十进制越多，字符串的字符也就越多，需要占用的空间越大**

**fwrite函数**

> 以二进制形式保存文件，读取成功返回size_t count

```c
size_t fwrite(const void*buffer, size_t size, size_t count, FILE*stream)
```

1. void*buffer 待写入文件数据的首地址，接受任何类型的指针
2. size_t size 每一块数据的大小
3. size_t 一共有多少块数据
4. FILE*stream文件结构指针
...
void printBinary(unsigned char dec)
{
    ...
    for (int i = count - 1; i>=0;i--)  // 将二进制进行正确的排列防止颠倒二读错
        print("%d", bits[i])
}
```
...
| 符号  | 意思                                      | 示例(输出结果为1或者0)     |
| ----- | ----------------------------------------- | -------------------------- |
| &&    | 将&&左右两边数据进行对比                  | 两边同为真即为真，否则为假 |
| \|\|  | 将\|\|两边数据进行对比                    | 两边为假即为假，否则为真   |
| !(非) | !右边为假，则为真如!0，而类似!12345则为假 | 假假为真，假真为假         |

**位逻辑运算符**

深入到二进制，将十进制数据转换为二进制，比对哪一项同为1或者0，抑或是相同，按和、或、非的思想去想

| 符号     | 意思                                     | 例子                                         |
| -------- | ---------------------------------------- | -------------------------------------------- |
| &        | 如果两个位同时为真，1表示，否则则为假为0 | 170->10101010&102->01100110 = 00100010->34   |
| \|       | 同为真，不同也是真，同为假则为假         | 170->10101010\|102->01100110 = 11101110->238 |
| ^(异或)  | 两个不同为真否则为假                     | 170->10101010^102->01100110 = 11001100->204  |
| ~(非)    | 将二进制翻转过来1->0, 0->1               | 170->10101010~01010101->85                   |
| <<(左移) | 将二进制位往左移动(231<<1)、(231<<2)     | 232->11100111 << 11001110(不够的往后面补0)   |
| >>(右移) | 将二进制往右移动(231>>1)                 | 232->11100111>>01110011                      |

## 二十九. 动态内存管理

### 29.1 内存的相关东西

**一段内存实际的构造是由4个区段栈(Stack)、堆(Heap)、放全局变量的区域(Static/Global)、以及放置指令的区域(Code/Text)所组成**

|               一般内存运用区域                |
| :-------------------------------------------: |
|              Heap(堆/Free Store)              |
|       Stack(栈)、Function to variables        |
| Static/Global(一般文件域的变量，例如全局变量) |
|               Code(指令之类的)                |

**NOTICE:**

> 1. 栈的空间一般使用要比标明的大(一般说的那种)
> 2. 堆是内存的空闲区，用于存储暂时不用的东西--->动态内存
>
> **无穷的递归容易导致栈的溢出**

### 29.2 要在C中使用动态内存的主要四个函数

**C: malloc、calloc(向后兼容)、reallor、free** 
...
int main()
{
	int a;
    int *p;
    p = (int*)malloc(sizeof(int));  // malloc(sizeof(int)):give 4 bytes space
    *p = 10;
    free(p);
    p = (int*)malloc(sizeof(int));  // (int*)强制转化，在C++中内存所存储的内型必须强制转化
    *p = 20
}
```

注：malloc找不到空的内存的时候-->不能在堆上成功分配内存-->将会返回NULL

如果是定义一个数组的时候的动态内存规划：最好在sizeof前面加上你要分配的字节长度P = (int*)malloc(20*sizeof(int))

### 29.3 什么是内存泄漏

> 1. C：malloc(存储至动态内存)---free(释放特定内存)
> 2. C++：new(获得动态内存)----delete(释放内存)

**内存泄漏**：申请使用动态内存，使用完之后不去释放，假如一个函数运行一百次，堆将会超出内存就会导致内存爆炸
...
使用cmd--->打入ping.exe--->会显示一些操作

> ping用于向目标主机发送一个网络请求，目标主机如果收到该请求将返回一个回应
...
> 1. argc：参数数量(argument count)的缩写
> 2. argv：参数值(argument value)的缩写

以在cmd的命令**ping baidu.com -t为例

| 使用指针移动和取值运算符  | 使用下标                     |
| ------------------------- | ---------------------------- |
| char**——>char *——>char[]  | char** ——> char * ——> char[] |
| argv ——> *argv ——> ping   | argv ——> argv[0] ——> ping    |
| *(argv + 1) ——> baidu.com | argv[1] ——> baidu.com        |
| *(argv + 2) ——> -t        | argv[2] ——> -t               |

其实主函数参数主要是要在命令行打当中使用的
